...
public:
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (currArg == "--help" || currArg == "-h") {
        ...
        if (numPositional >= positionalArgs_.size()) {
            throw std::invalid_argument(std::string("Invalid positional argument: ") + argv[i]);
        }
  ...
  template <typename T> T get(std::string_view name) {
    ...
    throw std::invalid_argument(std::string("Failed retrieving ") +
                                std::string(name));
  }
};